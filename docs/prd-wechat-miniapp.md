...
| 变更说明   | v0.1.1：认证方案对齐架构 AD-13——由"沿用手动输入、不引入 openid"调整为"openid 绑定 phone（wx.login 静默登录 + 首登手动输手机号）"，新增 `POST /api/wechat-login` 端点。其余（Taro 框架、核心+历史范围、小程序为主、暂不备案先内测）不变。 |
...
本期小程序覆盖 **6 个屏幕**（核心闭环 + 历史），对应 Web 的 F-01~F-05：
...
**明确不搬**：错题集页（§2.7）、错题试卷生成页（§2.8）→ 对应 **F-06、F-07 本期暂缓**，仍仅在 Web 可用。
...
- 错题集（F-06）与错题试卷生成（F-07）—— 本期不搬入小程序，仍仅 Web 可用。
...
2. **错题能力阶段性缺失**：小程序端暂无错题集/错题试卷，家长需回 Web 查看。与"小程序为主"定位存在阶段性矛盾，应在正式备案后作为下一期优先级补齐。
