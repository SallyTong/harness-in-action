...
export default function HistoryPage() {
  ...
  useEffect(() => {
    ...
    apiGet<SubmissionListResponse>(`/api/submissions?${params.toString()}`)
      .then((data) => {
        setSubmissions((prev) => (offset === 0 ? data.items : [...prev, ...data.items]));
        setTotal(data.total);
      })
      .catch((err) => {
  ...
  }, [childFilter, subjectFilter, offset, retryKey]);
  ...
  return (
    <div className="mx-auto flex min-h-dvh max-w-lg flex-col px-4 pb-16">
      ...
      <header className="flex items-center py-4">
        <h1 className="text-[22px] font-semibold leading-[30px] text-text-primary">批改历史</h1>
      </header>
          ...
          <>
                  ...
                  <div className="flex flex-1 flex-col min-w-0">
                    ...
                    <div className="mt-1 flex items-center justify-between">
                      {item.score ? (
                        ...
                        <span className="text-[13px] text-text-tertiary">批改中…</span>
                      )}
            ...
            {hasMore && (
              <div className="mt-4 pb-4">
                ...
                {loadMoreError && (
                  <p className="mt-2 text-center text-[13px] text-error">{loadMoreError}</p>
                )}
              </div>
            )}
          </>
