...
**How to apply:** When adding a new `_build_image_url` helper or calling an existing one, always pass the parent's phone. The reference implementation in `apps/backend/app/routers/submissions.py` already does this correctly.

**Files affected:**
- `apps/backend/app/routers/submissions.py` — `_build_image_url` already correct
- `apps/backend/app/routers/error_collections.py` — fixed 2026-08-09
