...
- Zero hardcoded secrets. GLM_API_KEY / QWEN_API_KEY, DB passwords from env vars only.
- Vision grading via the `VisionModel` abstraction (`app/services/vision/`, factory `get_vision_model()`). Select provider via `VISION_PROVIDER` (glm|qwen) + `VISION_MODEL`; log token usage per call tagged with `provider` + `model`.
...
- Grading flow: `POST /api/submissions` → 202 Accepted → BackgroundTasks → VisionModel → annotate → sync ErrorQuestion → status=completed|failed. Frontend polls every 2s.
